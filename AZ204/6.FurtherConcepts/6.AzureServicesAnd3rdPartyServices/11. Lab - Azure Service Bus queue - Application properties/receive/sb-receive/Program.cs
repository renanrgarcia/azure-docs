...
static async Task ReceiveMessages(string connectionString,string queueName,int maxMessages)
{
    ...
    foreach(var msg in messages)
    {
        ...
        string courseCategory = props.ContainsKey("courseCategory") ? props["courseCategory"].ToString() : "Unknown";
        string messageType = props.ContainsKey("messageType") ? props["messageType"].ToString() : "Unknown";
        ...
        Console.WriteLine($"Course Category: {courseCategory}, Message Type: {messageType}");                
